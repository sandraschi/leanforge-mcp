...
Write-Host "  Backend exe: $([math]::Round((Get-Item $src).Length / 1MB, 1)) MB" -ForegroundColor Green
...
if (Test-Path $envExample) {
    ...
    Write-Host "  WARNING: .env.example not found at repo root — create one before shipping" -ForegroundColor DarkYellow
}
...
$strayExe = "$PSScriptRoot\target\release\leanforge-mcp-backend.exe"
