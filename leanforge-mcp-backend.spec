...
# PyInstaller spec — produce leanforge-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller leanforge-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/leanforge_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/leanforge_mcp", "leanforge_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
